			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<a href="{{ url('admin/attractions/create') }}">Thêm mới</a>
						<div class="bootstrap-table">
							<div class="fixed-table-toolbar">
								<div class="pull-right search">
									{!! Form::open(['method' => 'GET', 'url' => 'admin/attractions']) !!}
						    		...
						    		<thead>
						    			...
						    			<tbody>
						    				@if(isset($attractions))
						    					@foreach($attractions as $key => $item)
						    						<tr>
						    							<td><input type="checkbox" name=""></td>
						    							<td>{{ $key+1 }}</td>
						    							<td style="width:150px; height:100px; ">
						    								<img src="{{ url('uploads/attractions/'.$item->image) }}" style="width:100%; height:100%; ">
						    							</td>
						    							<td>{{ $item->title }}</td>
						    							<td>
						    								<a href="{{ url('admin/attractions/'.$item->id.'/edit') }}" class="btn" style="float:left;">Edit</a>
						    							{!! Form::open(['method' => 'DELETE', 'url' => 'admin/attractions/'.$item->id]) !!}
						    								<a href="{{ url('admin/attractions/'.$item->id.'/delete') }}"><button style="background:none;" type="submit" class="btn" onclick="return confirm('Are you sure?')">Delete</button></a>
						    							{!! Form::close() !!}	
						    							</td>
						    						</tr>
						    					@endforeach
